...
case "$STRATEGY" in
    ...
    prism-ddg)
        ...
        PYTHONPATH="$ROOT/tools" python3 -m ddg probe-adv "$TARGET/${NAME}_ddg.json" "$TARGET/${NAME}_layout.json" --json "$TARGET/${NAME}_weights.json"
        CMD=(
            cargo run --bin prism-ddg --manifest-path "$ROOT/icsprism/Cargo.toml" --
            --ddg "$TARGET/${NAME}_ddg.json"
            --layout "$TARGET/${NAME}_layout.json"
            --weights-json "$TARGET/${NAME}_weights.json"
            "$@"
        )
        ;;
    prism-ddg-not-dumb)
